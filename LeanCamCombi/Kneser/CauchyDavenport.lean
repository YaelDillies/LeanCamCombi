...
open Finset Function Monoid MulOpposite OrderDual Subgroup
...
@[to_additive
      "The relation we induct along in the proof of Cauchy-Davenport theorem.\n`(s₁, t₁) < (s₂, t₂)` iff\n* `|s₁ + t₁| < |s₂ + t₂|`\n* or `|s₁ + t₁| = |s₂ + t₂|` and `|s₂| + |t₂| < |s₁| + |t₁|`\n* or `|s₁ + t₁| = |s₂ + t₂|` and `|s₁| + |t₁| = |s₂| + |t₂|` and `|s₁| < |s₂|`."]
    ...
    DevosMulRel x y ↔
      (x.1 * x.2).card < (y.1 * y.2).card ∨
        (x.1 * x.2).card = (y.1 * y.2).card ∧ y.1.card + y.2.card < x.1.card + x.2.card ∨
          (x.1 * x.2).card = (y.1 * y.2).card ∧
            x.1.card + x.2.card = y.1.card + y.2.card ∧ x.1.card < y.1.card := by simp [DevosMulRel, Prod.lex_iff, and_or_left]
...
lemma wellFoundedOn_devosMulRel :
  ...
  refine' IsWellFounded.wf.onFun.wellFoundedOn.prod_lex_of_wellFoundedOn_fiber fun n =>
    Set.WellFoundedOn.prod_lex_of_wellFoundedOn_fiber _ fun n =>
      IsWellFounded.wf.onFun.wellFoundedOn
  exact IsWellFounded.wf.onFun.wellFoundedOn.mono' fun x hx y _ => tsub_lt_tsub_left_of_le <|
...
lemma Finset.min_le_card_mul (hs : s.Nonempty) (ht : t.Nonempty) :
  ...
  · have hS : (zpowers g : Set α) ⊆ a⁻¹ • (s : Set α) := by
      refine'
        forall_mem_zpowers.2
          (@zpow_induction_right _ _ _ (· ∈ a⁻¹ • (s : Set α)) ⟨_, ha, inv_mul_self _⟩
      ...
      · simp only
        rwa [← op_smul_eq_mul, op_inv, ← Set.mem_smul_set_iff_inv_smul_mem, smul_comm,
          ←coe_smul_finset, hsg]
    exact Or.inl ((minOrder_le_nat_card (zpowers_ne_bot.2 hg) <|
      s.finite_toSet.smul_set.subset hS).trans <| WithTop.coe_le_coe.2 <|
        ((Nat.card_mono s.finite_toSet.smul_set hS).trans_eq <| by simp; sorry).trans <|
  ...
  replace hsg : (s ∩ op g • s).card < s.card :=
    card_lt_card
  ...
  · exact
      (ih _ _ (hgs.mono inter_subset_union) hgt <| devosMulRel_of_le aux2 hstg).imp
