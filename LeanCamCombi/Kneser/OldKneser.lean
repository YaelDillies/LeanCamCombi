...
lemma mulStab_union (hs₁ : (s ∩ a • C.mulStab).Nonempty) (ht₁ : (t ∩ b • C.mulStab).Nonempty)
  ...
  suffices h : Disjoint (x • C) (s ∩ a • C.mulStab * (t ∩ b • C.mulStab)) by
  ...
  have hUn :
    ...
    have : (x • C.biUnion fun y => y • C.mulStab) = C.biUnion fun y => x • y • C.mulStab :=
      biUnion_image
    simpa only [← this, biUnion_smul_finset, smul_eq_mul, mul_mulStab]
  ...
  clear hxyC hxyCsubC hs₁ ht₁ hC hCne hx h hy hyne
