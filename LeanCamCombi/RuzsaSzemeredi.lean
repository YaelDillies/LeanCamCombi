...
@[simp]
lemma ruzsa_szmeredi_number_nat_zero : ruzsaSzemerediNumberNat 0 = 0 :=
  le_zero_iff.1 ruzsaSzemerediNumberNat_le

@[simp]
lemma ruzsa_szmeredi_number_nat_one : ruzsaSzemerediNumberNat 1 = 0 :=
  le_zero_iff.1 ruzsaSzemerediNumberNat_le

@[simp]
lemma ruzsa_szmeredi_number_nat_two : ruzsaSzemerediNumberNat 2 = 0 :=
  le_zero_iff.1 ruzsaSzemerediNumberNat_le
...
@[simp]
lemma mem_triangleIndices : x ∈ triangleIndices s ↔ ∃ y, ∃ a ∈ s, (y, y + a, y + 2 * a) = x := by
  simp [triangleIndices]

@[simp]
lemma card_triangleIndices : #(triangleIndices s) = card α * #s := by
  simp [triangleIndices, card_univ]

lemma noAccidental (hs : ThreeAPFree (s : Set α)) :
...
lemma locallyLinear (hs : ThreeAPFree (s : Set α)) : (graph <| triangleIndices s).LocallyLinear :=
  haveI := noAccidental hs
  TripartiteFromTriangles.locallyLinear _

lemma card_edgeFinset (hs : ThreeAPFree (s : Set α)) [DecidableEq α] :
