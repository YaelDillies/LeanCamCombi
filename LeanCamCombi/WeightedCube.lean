...
import Project.Mathlib.Pmf
import Mathbin.Probability.IdentDistrib
import Project.Mathlib.Probability.Independence

#align_import weighted_cube
...
protected theorem ne_zero [Nonempty α] : μ ≠ 0 :=
...
protected theorem aEMeasurable (a : α) : AEMeasurable (fun ω => a ∈ X ω) μ := by
  classical
...
@[simp]
protected theorem nullMeasurableSet (a : α) : NullMeasurableSet {ω | a ∈ X ω} μ :=
  by
  rw [(by ext; simp : {ω | a ∈ X ω} = (fun ω => a ∈ X ω) ⁻¹' {True})]
  exact
    (bernoulli_seq.ae_measurable X _ a).nullMeasurableSet_preimage MeasurableSpace.measurableSet_top

protected theorem identDistrib (a j : α) : IdentDistrib (fun ω => a ∈ X ω) (fun ω => X ω j) μ μ :=
...
@[simp]
theorem meas_apply (a : α) : μ {ω | a ∈ X ω} = p :=
  by
...
protected theorem meas [Fintype α] (s : Finset α) :
...
instance compl : BernoulliSeq (fun ω => X ωᶜ) (1 - p) μ
    where
  le_one := tsub_le_self
  iIndepFun := by
...
protected theorem inter (h : IndepFun X Y μ) : BernoulliSeq (fun ω => X ω ∩ Y ω) (p * q) μ :=
...
independent `1 - p`-Bernoulli random variables. -/
protected theorem union (h : IndepFun X Y μ) :
    BernoulliSeq (fun ω => X ω ∪ Y ω) (p + q - p * q) μ :=
  by
