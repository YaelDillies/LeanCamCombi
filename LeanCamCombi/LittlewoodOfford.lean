...
#align_import littlewood_offord
...
theorem exists_littlewood_offord_partition [DecidableEq ι] (hr : 0 < r) (hf : ∀ i ∈ s, r ≤ ‖f i‖) :
    ∃ P : Finpartition s.powerset,
      P.parts.card = s.card.choose (s.card / 2) ∧
        (∀ 𝒜 ∈ P.parts, ∀ t ∈ 𝒜, t ⊆ s) ∧
          ∀ 𝒜 ∈ P.parts,
            (𝒜 : Set (Finset ι)).Pairwise fun u v => r ≤ dist (∑ i in u, f i) (∑ i in v, f i) :=
  by
  classical
...
/-- **Kleitman's theorem** -/
theorem card_le_of_forall_dist_sum_le (hr : 0 < r) (h𝒜 : ∀ t ∈ 𝒜, t ⊆ s) (hf : ∀ i ∈ s, r ≤ ‖f i‖)
