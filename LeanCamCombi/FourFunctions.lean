...
/-!
# The four functions lemma and FKG inequality
...
protected lemma LinearOrder.logSuperModular [LinearOrder α] [CommSemigroup β] [Preorder β]
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (s «expr ⊆ » u) -/
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (s «expr ⊆ » u) -/
/-- The **four functions lemma** -/
lemma four_functions_lemma' {u : Finset α} (h₁ : ∀ (s) (_ : s ⊆ u), 0 ≤ f₁ s)
    ...
    (∑ a in s, f₁ a) * ∑ b in t, f₂ b ≤ (∑ a in s ⊻ t, f₃ a) * ∑ b in s ⊼ t, f₄ b := by
...
/-- The **four functions lemma** -/
lemma four_functions_lemma {s t u : Finset α} (h₁ : ∀ a ∈ u, 0 ≤ f₁ a) (h₂ : ∀ a ∈ u, 0 ≤ f₂)
