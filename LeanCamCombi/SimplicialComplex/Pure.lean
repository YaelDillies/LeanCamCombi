...
def Pure (K : SimplicialComplex 𝕜 E) (n : ℕ) : Prop :=
  (∀ ⦃s : Finset E⦄, s ∈ K → #s ≤ n + 1) ∧ K.facets.Sized (n + 1)

def FullDimensional (K : SimplicialComplex 𝕜 E) : Prop := K.Pure (K.dim + 1)

lemma Pure.card_le (hK : K.Pure n) (hs : s ∈ K) : #s ≤ n + 1 := hK.1 hs
...
lemma Pure.mem_facets_iff (hK : K.Pure n) (hs : s ∈ K) : s ∈ K.facets ↔ #s = n + 1 :=
...
`n`-dimensional face. -/
lemma pure_iff : K.Pure n ↔ ∀ ⦃s⦄, s ∈ K → ∃ t ∈ K, #t = n + 1 ∧ s ⊆ t := by
