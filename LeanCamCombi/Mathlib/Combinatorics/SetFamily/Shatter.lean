...
protected lemma Shatter.of_compression (hs : (𝓓 a 𝒜).Shatter s) : 𝒜.Shatter s := by
  intros t ht
  obtain ⟨u, hu, rfl⟩ := hs ht
...
def StronglyShatter (𝒜 : Finset (Finset α)) (s : Finset α) : Prop :=
...
def OrderShatter : Finset (Finset α) → List α → Prop
  | 𝒜, [] => 𝒜.Nonempty
  | 𝒜, a :: l => (𝒜.nonMemberSubfamily a).OrderShatter l ∧ (𝒜.memberSubfamily a).OrderShatter l
      ∧ ∀ ⦃s : Finset α⦄, s ∈ 𝒜.nonMemberSubfamily a → ∀ ⦃t⦄, t ∈ 𝒜.memberSubfamily a →
        s.filter (a < ·) = t.filter (a < ·)

instance : DecidablePred 𝒜.OrderShatter
  | [] => decidableNonempty
  | a :: l => by unfold OrderShatter; sorry

def orderShatterer (𝒜 : Finset (Finset α)) : Finset (Finset α) :=
  (𝒜.biUnion powerset).filter $ fun s ↦ 𝒜.OrderShatter $ s.sort (· ≤ ·)
