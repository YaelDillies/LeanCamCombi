...
This file defines UW-compression. It is an operation on a set family that reduces its shadow.

UW-compressing `a : α` along `u v : α` means replacing `a` by `(a ⊔ u) \ v` if `a` and `u` are
disjoint and `v ≤ a`. In some sense, it's moving `a` from `v` to `u`.

UW-compressions are immensely useful to prove the Kruskal-Katona theorem. The idea is that
compressing a set family might decrease the size of its shadow, so iterated compressions hopefully
minimise the shadow.

## Main declarations

* `UW.compress`: `compress u v a` is `a` compressed along `u` and `v`.
* `UW.compression`: `compression u v s` is the compression of the set family `s` along `u` and `v`.
  ...
  `UW.card_compression`.
* `UW.card_shadow_compression_le`: Compressing reduces the size of the shadow. This is a key fact in
...
Even though our emphasis is on `Finset α`, we define UW-compressions more generally in a generalized
...
compression, UW-compression, shadow
-/
...
/-! ### UW-compression in generalized boolean algebras -/
...
/-- UW-compressing `a` means removing `v` from it and adding `u` if `a` and `u` are disjoint and
`v ≤ a` (it replaces the `v` part of `a` by the `u` part). Else, UW-compressing `a` doesn't do
...
theorem compress_mem_compression_of_mem_compression (ha : a ∈ 𝓓 u v s) : compress u v a ∈ 𝓓 u v s := by
...
/-- UW-compression reduces the size of the shadow of `𝒜` if, for all `x ∈ u` there is `y ∈ v`
such that `𝒜` is `(u.erase x, v.erase y)`-compressed. This is the key UW-compression fact needed for
Kruskal-Katona. -/
