...
@[simp]
lemma disjoint_edgeSet : Disjoint G.edgeSet H.edgeSet ↔ Disjoint G H := by
  rw [Set.disjoint_iff, disjoint_iff_inf_le, ← edgeSet_inf, ← edgeSet_bot, ←Set.le_iff_subset,
    OrderEmbedding.le_iff_le]

@[simp]
lemma nonempty_edgeSet : G.edgeSet.Nonempty ↔ G ≠ ⊥ := by
  rw [Set.nonempty_iff_ne_empty, ← edgeSet_bot,  edgeSet_inj.ne]

@[simp]
lemma disjoint_fromEdgeSet : Disjoint G (fromEdgeSet s) ↔ Disjoint G.edgeSet s := by
...
@[simp]
lemma fromEdgeSet_disjoint : Disjoint (fromEdgeSet s) G ↔ Disjoint s G.edgeSet := by
  rw [disjoint_comm, disjoint_fromEdgeSet, disjoint_comm]

@[simp]
lemma deleteEdges_eq : G.deleteEdges s = G ↔ Disjoint G.edgeSet s := by
