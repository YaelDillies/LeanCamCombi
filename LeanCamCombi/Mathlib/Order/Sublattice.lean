...
lemma closure_preimage_le (f : LatticeHom α β) (s : Set β) :
...
lemma map_comap_eq_self {f : LatticeHom α β} {L : Sublattice β} (h : L ≤ f.range) :
    map f (comap f L) = L := by rwa [map_comap_eq, inf_eq_right]
...
lemma comap_map_eq_self {f : LatticeHom α β} (h : f.ker ≤ L) : comap f (map f L) = L := by
...
lemma map_le_map_iff {f : LatticeHom α β} {L M : Sublattice α} :
    L.map f ≤ L.map f ↔ L ≤ L ⊔ f.ker := by rw [map_le_iff_le_comap, comap_map_eq]
...
lemma map_eq_range_iff {f : LatticeHom α β} : L.map f = f.range ↔ Codisjoint L f.ker := by
