...
import LeanCamCombi.Mathlib.LinearAlgebra.Matrix.SpecialLinearGroup
...
lemma fact_3_1_2 : (X ^ n).card ≤ X.card ^ n := card_pow_le
...
def HasPolynomialGrowth : Prop :=
  ∃ X : Finset G, X⁻¹ = X ∧ Subgroup.closure (X : Set G) = ⊤ ∧ ∃ d, ∀ n ≥ 2, (X ^ n).card ≤ n ^ d

/-- Gromov. -/
lemma theorem_3_2 : HasPolynomialGrowth G ↔ IsVirtuallyNilpotent G := sorry

lemma fact_3_3 [Fintype G] (hn : X ^ n = univ) : log (card G) / log X.card ≤ n := by
...
lemma proposition_3_7 :
    ∃ ε > 0, ∀ X : Finset SL(2, ℝ), (X ^ 2).card ≤ 1000 * X.card → (∀ M ∈ X, ∀ i j, |M i j| ≤ ε) →
...
lemma theorem_3_9 :
    ∃ δ > 0, ∃ ε > 0,
      ∀ k [Field k] [Fintype k] [DecidableEq k] (A : Finset SL(n, k)) (hAgen : Subgroup.closure (A : Set SL(n, k)) = ⊤),
        A.card ^ (1 + δ) ≤ (A ^ 3).card ∨ Fintype.card SL(n, k) ^ (1 - ε) ≤ A.card := sorry
...
lemma lemma_3_11 (hA : σₘ[A] < 3 / 2) :
    ∃ (H : Subgroup G) (_ : Fintype H),
      card H < 3 / 2 * A.card ∧ ∀ a ∈ A, (A : Set G) ⊆ a • H ∧ a • (H : Set G) = op a • H := sorry
